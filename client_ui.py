...
class StudyCirclesApp(ctk.CTk):
    def __init__(self):
        ...
        self.backend = ChatClient(message_callback=self._handle_inbound_dm, start_terminal_listener=False)
            ...
            while True:
                try:
                    ...
                    if p_type in ["AUTH_SUCCESS", "AUTH_FAIL"]:
                        ...
                        self.backend._log_message_locally(chat_partner=packet["sender"], sender=packet["sender"], text=decrypted)
                        ...
                        for nested_packet in packet.get("content", []):
                            t = nested_packet.get("type")
                            if t == "DIRECT_MSG":
                                decrypted = self.backend.decrypt_msg(nested_packet["content"])
                        ...
                        self.after(1, lambda: self.trigger_manual_refresh())
    ...
    def create_group_dialog(self):
        dialog = ctk.CTkInputDialog(text="Enter unique name layout for New Group room channel:", title="Create Group")
    ...
    def open_chat_hook(self, target_name, target_type):
        ...
        saved_history = self.backend.get_local_history(target_name)
        self.active_conversations[target_name] = saved_history
        
        # 2. Render out historical lines
    ...
    def send_message_hook(self):
        ...
        if self.current_target not in self.active_conversations:
            self.active_conversations[self.current_target] = []
        self.active_conversations[self.current_target].append((self.backend.username, message))

        def async_outbound_dispatch():
            try:
                if self.current_target_type == "user":
                    self.backend.send_dm(self.current_target, message)
                    # Render DMs locally since server doesn't echo them back
